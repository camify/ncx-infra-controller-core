...
pub async fn handle_bom_validating(
    ...
    match bom_validating_state {
        BomValidatingState::BomValidateComplete => {
            tracing::info!("BOM Validating Switch: BomValidateComplete, moving to Ready");
            Ok(StateHandlerOutcome::transition(SwitchControllerState::Ready))
        }
    }
}
